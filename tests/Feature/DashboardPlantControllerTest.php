...
it('retorna as plantas associadas ao utilizador', function () {
    ...
    // Crie algumas plantas e associe-as ao usuário
    ...
    // Faça a requisição para obter as plantas do usuário
    $response = $this->actingAs($user)->getJson("/api/user-plants/{$user->id}");

    // Verifique se a resposta tem status 200 e as plantas associadas
...
it('can get user plants with kits', function () {
    ...
    // Faça a requisição para obter as plantas do usuário com os kits
    $response = $this->actingAs($user)->getJson("/api/user-plants-kits/{$user->id}");

    // Verifique se a resposta tem status 200
    $response->assertOk();

    // Verifique a estrutura JSON da resposta
...
it('returns 404 if plant not found by ID', function () {
    ...
    // Faça a requisição autenticando o usuário e com um ID de planta inexistente
