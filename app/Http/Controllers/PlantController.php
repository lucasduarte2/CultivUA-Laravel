...
class PlantController extends Controller
{
    ...
    {
        // Retornar as plantas do usuário autenticado
        \Log::info('Iniciando requisição para obter plantas do usuário autenticado.');
        try {
            $plants = auth()->user()->plants;
            \Log::info('Plantas obtidas com sucesso', ['plants_count' => count($plants)]);
            return response()->json($plants, 200);
        } catch (\Exception $e) {
            \Log::error('Erro ao obter plantas do usuário', ['exception' => $e->getMessage()]);
    ...
    {
        \Log::info('Iniciando a execução do método searchPlant');
        // Validação dos dados recebidos na requisição
        \Log::info('Iniciando a validação dos dados de pesquisa de planta.', ['request_data' => $request->all()]);
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                \Log::error('Erro ao buscar plantas', ['status' => $response->status()]);
                return response()->json(['error' => 'Erro ao buscar plantas.'], $response->status());
            }
        } catch (\Exception $e) {
    ...
    {
        \Log::info('Iniciando a adição de planta à dashboard', ['request_data' => $request->all()]);
        ...
        try {
            ...
            if (!$plant) {
                \Log::info('Planta não encontrada, consultando a API externa.', ['plant_id' => $plantId]);
            ...
            \Log::info('Planta associada com sucesso ao usuário', ['user_id' => $userId, 'plant_id' => $plant->id]);
    ...
    {
        \Log::info('Iniciando execução do fetchPlantFromApi', ['plant_id' => $plantId]);
        \Log::info('Consultando API externa para dados da planta', ['plant_id' => $plantId]);

        $apiKey = "sk-8hd36760119ba6b308001";
        ...
        try {
            if (isset($apiUrl) && $apiUrl !== null) {
                ...
                // Retorne uma resposta de erro apropriada para o usuário
            }
            ...
            if ($response->successful()) {
                ...
                if ($existingPlant) {
                    \Log::info('Planta já existente, associando ao usuário', ['plant_id' => $existingPlant->id]);
                ...
                \Log::info('Care Guides antes de salvar', ['guides' => json_encode($careGuides)]);
                ...
                \Log::info('Nova planta criada e associada ao usuário', ['plant_id' => $plant->id]);
    ...
    private function fetchCareGuide($speciesId)
    {
        ...
        \Log::info('Iniciando consulta de guias de cuidados', ['species_id' => $speciesId]);
        \Log::info('Chave da API para acesso à Perenual', ['apiKey' => $apiKey]);
        \Log::info('Enviando requisição para obter guias de cuidados', ['url' => "https://perenual.com/api/species-care-guide-list?key={$apiKey}&species_id={$speciesId}"]);
        ...
        \Log::info('Consultando guias de cuidados', ['species_id' => $speciesId]);
    ...
    private function fetchAndSaveFaqs($plantName, $plantId)
    {
        ...
        \Log::info('Consultando FAQs para planta', ['plant_name' => $plantName]);
        ...
        \Log::info('Consultando FAQs', ['api_url' => $apiUrl]);
        ...
        if ($response->successful()) {
            ...
            foreach ($faqs as $faq) {
                // Salva cada FAQ associando ao plantId
                PlantFaq::create([
                    'plant_id' => $plantId,
    ...
    public function identifyPlant(Request $request)
    {
        try {
            ...
            $apiKey = "oFBq2mzqsJhfeGG8FqYeB7g2917cxBZo2vh7muJ7sJLFDLuSD7";
            $perenualApiKey = "sk-8hd36760119ba6b308001";

            // Lendo o conteúdo da imagem e convertendo para binário
            ...
            // Coletando as coordenadas, caso estejam presentes
            ...
            if ($response->successful()) {
                ...
                \Log::info('Consultando API Perenual', ['url' => $perenualApiUrl]);
    ...
    {
        // Verificar se a planta pertence ao usuário autenticado
    ...
    public function store(Request $request)
    {
        ...
        // Criar planta associada ao usuário autenticado
